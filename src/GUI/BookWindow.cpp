...
BookWindow::BookWindow(const Item& item) : Window(
	sf::FloatRect(sf::Vector2f(0.f, 0.f), sf::Vector2f(WIDTH, HEIGHT)),
...
void BookWindow::setPosition(const sf::Vector2f& pos) {
	...
	m_leftArrow->setPosition(pos + sf::Vector2f(0.5f * WIDTH - 20.f - m_leftArrow->getBoundingBox()->size.x, HEIGHT - 50.f));
	m_rightArrow->setPosition(pos + sf::Vector2f(0.5f * WIDTH + 20.f, HEIGHT - 50.f));

	m_bookTitle.setPosition(pos + sf::Vector2f(0.5f * WIDTH - 0.5f * m_bookTitle.getLocalBounds().size.x, 0.3f * HEIGHT));
	m_title.setPosition(pos + sf::Vector2f(0.5f * WIDTH - 0.5f * m_title.getLocalBounds().size.x, GUIConstants::TOP));

	TextAlignment alignment = m_content.getTextAlignment();
	float contentX = (alignment == TextAlignment::Left) ? MARGIN :
		(alignment == TextAlignment::Right) ? WIDTH - m_content.getBounds().size.x - MARGIN :
		(WIDTH - m_content.getBounds().size.x) * 0.5f;
	...
	if (m_showSprite) {
		m_sprite->setPosition(pos + sf::Vector2f(
			0.5f * (WIDTH - m_sprite->getTextureRect().size.x),
			0.5f * (HEIGHT - 50.f - m_sprite->getTextureRect().size.y)));
	}
}
